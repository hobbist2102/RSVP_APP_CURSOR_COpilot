  ...
  try {
    ...
    if (stage === 1) {
      await db.update(guests)
        .set({
          rsvp_status: validatedData.attending === 'yes' ? 'attending' : 
                      validatedData.attending === 'no' ? 'not_attending' : 'maybe',
          updated_at: new Date()
        })
    ...
    if (stage === 2 || (stage === 1 && validatedData.attending === 'no')) {
      await RSVPTokenService.markTokenAsUsed(token)
    }

    // Determine next step
    const nextStep = stage === 1 && validatedData.attending === 'yes' ? 'stage2' : 'complete'
