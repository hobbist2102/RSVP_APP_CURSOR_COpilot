    ...
    try {
      ...
      const result = await db
        .select({
          ...
          guestId: rsvpTokens.guest_id,
          token: rsvpTokens.token,
          expiresAt: rsvpTokens.expires_at,
          usedAt: rsvpTokens.used_at,
          isActive: rsvpTokens.is_active,
          guest: {
            id: guests.id,
            ...
            plusOnesAllowed: guests.plus_one_allowed
          }
        })
        .from(rsvpTokens)
    ...
    try {
      ...
      if (eventId) {
        query = query.innerJoin(guests, 
          and(
            eq(rsvpTokens.guest_id, guests.id),
  ...
  static async cleanupExpiredTokens(): Promise<number> {
    try {
      const result = await db.update(rsvpTokens)
        ...
        .where(and(
          eq(rsvpTokens.is_active, true),
          lt(rsvpTokens.expires_at, new Date())
        ))
    ...
    try {
      const result = await db
        .select({
          ...
          expiresAt: rsvpTokens.expires_at,
          usedAt: rsvpTokens.used_at,
          createdAt: rsvpTokens.created_at,
          guest: {
            firstName: guests.first_name,
            lastName: guests.last_name,
            email: guests.email,
